...
namespace FiveChess
{
    class MyDraw
    {
        ...
        public MyDraw() { }
        public MyDraw(Rectangle rect, int num, List<Color> colors)
        {
        ...
        {
            ptLct= GetCursorPtF(ref padInfo.cursorArry, new PointF(pointF.X-30,pointF.Y-30));
            PointF ptF = new PointF();
            ptF.X = padInfo.cursorArry[ptLct.Y][ptLct.X].X;
            ptF.Y = padInfo.cursorArry[ptLct.Y][ptLct.X].Y;
            
            if (padInfo.isPiecesArry[ptLct.Y, ptLct.X] != 1)
            {
                grp.FillEllipse(new SolidBrush(pcsColor[nflag]), 
                    new RectangleF(p0.X + ptF.X - padInfo.pcsSize, p0.Y + ptF.Y - padInfo.pcsSize, 
                    padInfo.pcsSize * 2, padInfo.pcsSize * 2));
            }
            //else
            //    MessageBox.Show("此点已有棋子！");

            padInfo.isPiecesArry[ptLct.Y, ptLct.X] = 1;
            padInfo.pcsClsFlg[ptLct.Y, ptLct.X] = nflag;
        }
